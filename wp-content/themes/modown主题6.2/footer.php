...
<div class="rollbar">
	<ul>
		<?php if(!_MBT('vip_hidden') && (is_user_logged_in() || !_MBT('hide_user_all'))){?><li><a href="<?php echo get_permalink(MBThemes_page("template/vip.php"));?>"><i class="icon icon-crown"></i></a><h6>升级<?php $erphp_vip_name = get_option('erphp_vip_name')?get_option('erphp_vip_name'):'VIP'; echo $erphp_vip_name;?><i></i></h6></li><?php }?>
		<?php 
			if(_MBT('checkin')) {
		?>
...
<div class="footer-fixed-nav clearfix"<?php echo $footer_nav_style;?>>
	<?php if(_MBT('footer_nav_html')){
		...
		<a href="https://www.fulijidi.xyz/girls/"><i class="icon icon-find"></i><span>发现</span></a>
		<a href="<?php echo get_permalink(MBThemes_page("template/vip.php"));?>"><i class="icon icon-crown"></i><span>VIP</span></a>
		<a href="https://www.fulijidi.xyz/read/" rel="nofollow"><i class="icon icon-cat"></i><span>关于</span></a>
	<?php }?>
