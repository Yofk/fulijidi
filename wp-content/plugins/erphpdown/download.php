...
if($key){
		...
		if($filepath){
			...
			if(strpos($filepath,',')){
				...
				if($arrlength == 1){
					...
					if($arrlength >= 2){
						$downMsg.="<p><span class='tit'>".$filearr[0]."</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link erphpdown-down-btn' data-clipboard-text='".$filearr3[1]."'>点击下载</a>".$filearr3[0].": <span class='erphpdown-code'>".trim($filearr3[1])."</span><a class='erphpdown-copy' data-clipboard-text='".trim($filearr3[1])."' href='javascript:;'>复制</a></p>";
					}else{
	...
	if($hiddens){
		...
		if($erphp_hide_style){
			$downMsg .='<div class="title"><span>隐藏信息</span></div><div class="hidden-content">';
