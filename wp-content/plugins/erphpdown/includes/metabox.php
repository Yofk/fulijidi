...
function erphpdown_metaboxs() {
	...
	$meta_boxes = array(
		array(
			"name"             => "erphp_down",
			"title"            => "收费模式",
			"desc"             => "可以通过短代码<code>[erphpdown]隐藏内容[/erphpdown]</code>隐藏部分收费内容，短代码<code>[vip]隐藏内容[/vip]</code>隐藏VIP内容",
			"type"             => "downradio",
		...
		array(
			"name"             => "member_down",
			"title"            => "VIP优惠",
			"desc"             => "专享指VIP免费、普通用户无法购买，专享购买指仅VIP可购买，高等级享有低等级的权限",
			"type"             => "vipradio",
			'options' => array(
				'1' => '无',
	            '4' => '专享',
            		...
            		if($cnt){
            			for($i=0; $i<$cnt;$i++){
            				echo '<div class="down-url">
								<input type="number" min="1" step="1" name="down_urls[index][]" value="'.$value['index'][$i].'" placeholder="序号" style="width:8%" required><input type="text" name="down_urls[name][]" value="'.$value['name'][$i].'" placeholder="名称" style="width: 15%" required><input type="number" step="0.01" name="down_urls[price][]" value="'.$value['price'][$i].'" placeholder="价格" style="width: 10%"><div style="display: inline-block;width: 50%;"><textarea name="down_urls[url][]" placeholder="地址" style="width: 100%" rows="2">'.$value['url'][$i].'</textarea><a href="javascript:;" class="erphp-add-url button">上传媒体库文件</a> <a href="javascript:;" class="erphp-add-url2 button button-primary">上传本地文件</a> <span></span></div><select name="down_urls[vip][]" style="display:inline-block;"><option value="0">VIP默认优惠</option><option value="1" '.($value['vip'][$i] == '1'?'selected':'').'>无</option><option value="4" '.($value['vip'][$i] == '4'?'selected':'').'>VIP专享</option><option value="8" '.($value['vip'][$i] == '8'?'selected':'').'>年费VIP专享</option><option value="9" '.($value['vip'][$i] == '9'?'selected':'').'>终身VIP专享</option><option value="3" '.($value['vip'][$i] == '3'?'selected':'').'>VIP免费</option><option value="6" '.($value['vip'][$i] == '6'?'selected':'').'>年费VIP免费</option><option value="7" '.($value['vip'][$i] == '7'?'selected':'').'>终身VIP免费</option><option value="2" '.($value['vip'][$i] == '2'?'selected':'').'>VIP 5折</option><option value="5" '.($value['vip'][$i] == '5'?'selected':'').'>VIP 8折</option><option value="13" '.($value['vip'][$i] == '13'?'selected':'').'>VIP 5折|终身免费</option><option value="14" '.($value['vip'][$i] == '14'?'selected':'').'>VIP 8折|终身免费</option><option value="10" '.($value['vip'][$i] == '10'?'selected':'').'>VIP专享购买</option><option value="11" '.($value['vip'][$i] == '11'?'selected':'').'>专享购买|年费5折</option><option value="12" '.($value['vip'][$i] == '12'?'selected':'').'>专享购买|年费8折</option></select> <a href="javascript:;" class="del-down-url">删除</a>
							</div>';
            			}
            		}
				}
