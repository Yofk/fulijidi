				...
				if($start_down || $start_down2 || $start_see || $start_see2){
					...
					if($result)
					{
						...
						if(get_option('erphp_remind')){
							wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】订单提醒 - '.$subject, '用户'.$user_info->user_login.'消费'.$price.get_option('ice_name_alipay').'购买了'.$subject.get_permalink($postid));
						}
